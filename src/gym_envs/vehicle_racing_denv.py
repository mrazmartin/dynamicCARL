...
from carl.context.context_space import (
    ContextFeature, UniformIntegerContextFeature, UniformFloatContextFeature
)
...
class CARLVehicleRacing(CARLGymnasiumEnv):
    ...
    def get_context_features() -> dict[str, ContextFeature]:
        return {
            ...
            "COM_X": UniformFloatContextFeature(
                "COM_X", lower=-5.0, upper=5.0, default_value=0.0
            ),
            "COM_Y": UniformFloatContextFeature(
                "COM_Y", lower=-3.0, upper=3.0, default_value=0.0
            ),
        }

    def _update_context(self) -> None:
        self.env: CustomCarRacing
        self.context = CARLVehicleRacing.get_context_space().insert_defaults(
            self.context
        )
